...
import type { LearningStyle } from '@/services/storage/userProfile';
        ...
        <SafeAreaView className="flex-1 bg-white">
            <ScrollView className="flex-1" contentContainerStyle={{ padding: 24 }}>
                ...
                <AnimatedView entering={FadeInDown.delay(200).duration(600)} className="mb-8">
                    ...
                    <View className="flex-row flex-wrap gap-2">
                        {interestOptions.map((interest) => (
                            ...
                            >
                                <Text
                                    className={`${interests.includes(interest) ? 'text-primary-700' : 'text-gray-700'}`}
                                >
                                    {interest}
                                </Text>
                            </TouchableOpacity>
                    ...
                    {goalOptions.map((goal) => (
                        <TouchableOpacity
                            ...
                            className={`p-3 mb-2 rounded-xl border-2 ${
                                goals.includes(goal) ? 'border-primary-500 bg-primary-50' : 'border-gray-200 bg-white'
                            }`}
                        >
                            <Text className={`${goals.includes(goal) ? 'text-primary-700' : 'text-gray-700'}`}>
                                {goal}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                <AnimatedView entering={FadeInDown.delay(400).duration(600)} className="mb-8">
                    <Text className="text-lg font-semibold text-gray-900 mb-3">Preferred learning style</Text>
