...
import { achievementDefinitions, Achievement } from './achievements';
...
class GamificationService {
  ...
  async getAllAchievements(): Promise<
    Array<Achievement & { isUnlocked: boolean; unlockedAt?: number }>
  > {
    const state = await this.getState();

    return achievementDefinitions.map((achievement) => {
      const unlocked = state.unlockedAchievements.find(
